...
abstract class Vehicle implements Accelerateable {
  ...
  abstract accelerate(kmh: number): number;

  brake(): void {
    this.velocity = 0;
    console.log(
      `${this.name} has braked. The velocity is now ${this.velocity} km/h.`,
    );
  }
}

class Car2 extends Vehicle {
  accelerate(kmh: number) {
    const newVelocity = (this.velocity += kmh);
    console.log(
      `${this.name} is speeding up. The velocity is now ${newVelocity} km/h.`,
    );
    return newVelocity;
  }
}
...
class Bicycle extends Vehicle {
  accelerate(kmh: number) {
    ...
    if (newBikeVelocity > 35) {
      ...
      newBikeVelocity = 35;
      console.log(
        `${this.name} reached the maximum velocity of ${newBikeVelocity} km/h.`,
      );
      return newBikeVelocity;
    } else {
      this.velocity = newBikeVelocity;
      console.log(
        `${this.name} is speeding up. The velocity is now ${newBikeVelocity} km/h.`,
      );
      return newBikeVelocity;
    }
  }
